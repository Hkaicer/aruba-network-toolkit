...
if uploaded:
    ...
    st.dataframe(df_xcvr, use_container_width=True)
        ...
        # ===== Environment =====
    st.subheader("Environment (CPU temperatures)")
    st.dataframe(df_env, use_container_width=True)

    if not df_env.empty:
        # Plot only CPU temperature sensors (fallback to all temps if CPU not found)
        if "sensor" in df_env.columns:
            def _is_cpu_sensor(s: str):
                s = (s or "").lower()
                keywords = [
                    "cpu", "processor", "proc", "asic", "switch chip", "switchchip",
                    "management cpu", "mgmt cpu", "routing engine", "control plane"
                ]
                return any(k in s for k in keywords)

            # numeric temperature rows
            temps_all = df_env.dropna(subset=["value"]).copy()
            cpu_temps = temps_all[temps_all["sensor"].apply(_is_cpu_sensor)]
            plot_df = cpu_temps if not cpu_temps.empty else temps_all

            if not plot_df.empty:
                fig_t, ax_t = plt.subplots()
                ax_t.bar(plot_df["sensor"], plot_df["value"])
                ax_t.set_title("CPU Temperatures (°C)" if not cpu_temps.empty else "Temperature Sensors (°C)")
                ax_t.set_ylabel("°C")
